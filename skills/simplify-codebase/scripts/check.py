#!/usr/bin/env python3
"""Run release checks for the simplify-codebase skill."""
...
def content_check() -> bool:
    ...
    if len(skill.splitlines()) > 180:
        problems.append("SKILL.md exceeds 180 lines")
    if len(skill.encode("utf-8")) > 14_000:
        problems.append("SKILL.md exceeds 14,000 bytes")
    if "references/candidate-evidence.md" not in skill:
        problems.append("candidate evidence reference is not routed from SKILL.md")
    if "$simplify-codebase" not in metadata:
        problems.append("openai.yaml default prompt does not mention $simplify-codebase")
    if problems:
...
def main() -> int:
    ...
    checks = [content_check(), cache_check()]
    if QUICK_VALIDATE.exists():
        checks.append(run("skill validate", [sys.executable, "-B", str(QUICK_VALIDATE), str(SKILL_DIR.relative_to(REPO_ROOT))]))
    else:

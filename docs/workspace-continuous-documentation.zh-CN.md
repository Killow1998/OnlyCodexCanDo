...
把这些步骤看成一次轻量的阶段关闭，而不是额外仪式。完成后，一个新的开发者或 Agent 应该只读 `active/`、`design/` 和 `worklog/`，就能知道完成了什么、哪些结果已经验证、下一步是什么，不必读取上一段对话。不要把对话原文复制进 workspace 文档，也不要创建长期 `handoff` 作为第四个事实来源；小改动仍然不要求执行这套收尾。
